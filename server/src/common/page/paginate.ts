...
export function paginate<T>(repository: Repository<T>, options: IPaginationOptions,  searchOptions?: FindConditions<T> | FindManyOptions<T>) : Promise<Pagination<T>>
...
export async function paginate<T>(
  ...
  function resolveOptions(options: IPaginationOptions): [number, number] {
    const pageNumber =
      options.pageNumber >= 0 ? options.pageNumber : 0;
